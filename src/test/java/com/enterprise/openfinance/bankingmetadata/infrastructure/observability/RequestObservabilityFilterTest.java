package com.enterprise.openfinance.bankingmetadata.infrastructure.observability;
...
class RequestObservabilityFilterTest {
    ...
    void shouldAddTraceHeaderAndPublishMetrics() throws ServletException, IOException {
        ...
        MockHttpServletRequest request = new MockHttpServletRequest("GET", "/open-finance/v1/metadata/standing-orders");
        request.setAttribute(HandlerMapping.BEST_MATCHING_PATTERN_ATTRIBUTE, "/open-finance/v1/metadata/standing-orders");
        request.addHeader("X-FAPI-Interaction-ID", "ix-bankingmetadata-1");
        MockHttpServletResponse response = new MockHttpServletResponse();

        filter.doFilter(request, response, (req, res) -> ((MockHttpServletResponse) res).setStatus(200));

        assertThat(response.getHeader("X-Trace-ID")).isEqualTo("ix-bankingmetadata-1");
        assertThat(meterRegistry.find("openfinance_http_requests_total")
                .tags("service", "openfinance-banking-metadata-service", "method", "GET", "route",
                        "/open-finance/v1/metadata/standing-orders", "status", "200")
                .counter()).isNotNull();
    }
    ...
    void shouldUseTraceparentWhenAvailable() throws ServletException, IOException {
        ...
        request.addHeader("X-FAPI-Interaction-ID", "ix-bankingmetadata-2");
        MockHttpServletResponse response = new MockHttpServletResponse();
